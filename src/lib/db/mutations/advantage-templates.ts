...
import type { AdvantageTemplateInsert, AdvantageTemplateUpdate } from "@/types/database";
...
export async function updateAdvantageTemplate(
  id: string,
  patch: AdvantageTemplateUpdate,
): Promise<AdvantageTemplate> {
  const supabase = createServerSupabaseClient();
...
export async function upsertAdvantageTemplateByEffectCode(
  input: AdvantageTemplateInsert,
): Promise<AdvantageTemplate> {
  const supabase = createServerSupabaseClient();
  const { data, error } = await supabase
    .from("advantage_templates")
